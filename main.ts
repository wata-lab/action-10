...
controller.right.onEvent(ControllerButtonEvent.Pressed, function () {
    if (is_invincible == 0) {
        ...
        animation.runImageAnimation(
        mySprite,
        assets.animation`金いぬたぬき_アニメーション_右`,
        200,
        true
        )
    }
...
controller.left.onEvent(ControllerButtonEvent.Pressed, function () {
    if (is_invincible == 0) {
        ...
        animation.runImageAnimation(
        mySprite,
        assets.animation`金いぬたぬき_アニメーション_左`,
        200,
        true
        )
    }
...
function init_stage (stage: number) {
    ...
    if (stage == 0) {
        ...
        tiles.setCurrentTilemap(tilemap`レベル_8`)
        ...
        scene.setBackgroundImage(img`
            ...
            `)
        sprites.destroy(mySprite4)
        mySprite4 = sprites.create(assets.image`シルバーフィッシュ_生きてる_左`, SpriteKind.踏んだら死ぬ敵)
        tiles.placeOnTile(mySprite4, tiles.getTileLocation(7, 10))
        enemy_move = 0
    } else if (stage == 8) {
        sprites.destroy(mySprite4)
    ...
    if (respaun_column != 0 && respaun_row != 0) {
...
game.onUpdate(function () {
    if (stage == 3 && mySprite.tileKindAt(TileDirection.Bottom, assets.tile`ボタン_押されてない`)) {
        music.setVolume(128)
...
forever(function () {
    if (mySprite.tileKindAt(TileDirection.Bottom, assets.tile`ボタン_押されてない`) && stage == 8) {
        tiles.setTileAt(tiles.getTileLocation(3, 9), assets.tile`ボタン_押された`)
        l = 4
        for (let index = 0; index < 10; index++) {
            tiles.setTileAt(tiles.getTileLocation(l, 10), assets.tile`泡`)
            tiles.setWallAt(tiles.getTileLocation(l, 10), true)
            l += 1
        }
        pause(5000)
        l = 4
        for (let index = 0; index < 10; index++) {
            tiles.setTileAt(tiles.getTileLocation(l, 10), assets.tile`transparency16`)
            tiles.setWallAt(tiles.getTileLocation(l, 10), false)
            l += 1
        }
    }
})
